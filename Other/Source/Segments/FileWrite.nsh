...
${SegmentPrePrimary}
	...
	${Do}
		...
		${If} $0 == ConfigWrite
			...
			${EndIf}
			ClearErrors
			${ReadLauncherConfig} $8 FileWrite$R0 UnixEOL
			${If} $8 != true
			${AndIf} $8 != false
			${AndIfNot} ${Errors}
				${InvalidValueError} [FileWrite$R0]:UnixEOL $8
				${Continue}
			${EndIf}
		${ElseIf} $0 == INI
...
!endif
		...
		${ForEachFile} $1 $R4 $7
			${If} $0 == ConfigWrite
				${If} $4 == true
          ${If} $8 == true
            ${DebugMsg} "Writing configuration to a file with ConfigWriteUnixS.$\r$\nFile: $1$\r$\nEntry: `$2`$\r$\nValue: `$3`"
            ${ConfigWriteUnixS} $1 $2 $3 $R9
          ${Else} ; false or empty
          ...
          ${EndIf}
				${Else} ; false or empty
          ${If} $8 == true
            ${DebugMsg} "Writing configuration to a file with ConfigWriteUnix.$\r$\nFile: $1$\r$\nEntry: `$2`$\r$\nValue: `$3`"
            ${ConfigWriteUnix} $1 $2 $3 $R9
          ${Else} ; false or empty
